...
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender {
  if([[segue identifier] isEqualToString:@"ShowPropertiesSegue"]) {
    ...
    if([vc isKindOfClass:[GFSPropertiesViewController class]]) {
      ...
      [(GFSPropertiesViewController *)[nav topViewController] setProperties:[self propertiesForImage]];
    }
  }
}
...
- (UIImage *)cacheThumbnailImage {
  ...
  CFDictionaryRef properties = CGImageSourceCopyProperties(imgSrc, NULL);
  CFDictionaryRef imageProperties = CGImageSourceCopyPropertiesAtIndex(imgSrc,
                                                                       0,
                                                                       NULL);
  CGImageDestinationSetProperties(imgDest, properties);
  CGImageDestinationAddImage(imgDest, img, imageProperties);
...
- (NSDictionary *)propertiesForImage {
  ...
  CFDictionaryRef imageProperties = CGImageSourceCopyPropertiesAtIndex(imgSrc, 0, NULL);
  
  NSMutableDictionary *properties = [NSMutableDictionary dictionaryWithDictionary:(__bridge NSDictionary *)fileProperties];
